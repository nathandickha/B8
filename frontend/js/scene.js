...
const SPA_INTERIOR_CLEAR_INSET_XY = 0.05;
const SPA_INTERIOR_CLEAR_INSET_Z = 0.02;
...
function getSpaInteriorClearBox(spaGroup, insetXY = SPA_INTERIOR_CLEAR_INSET_XY, insetZ = SPA_INTERIOR_CLEAR_INSET_Z) {
  if (!spaGroup) return null;
  spaGroup.updateMatrixWorld?.(true);
  const box = new THREE.Box3().setFromObject(spaGroup);
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  // 4) Interior spa clear box (orange): removes non-spa geometry inside spa volume.
  ...
  // 5) Actual spa body extents (green) for reference only
...
export function updatePoolWaterVoid(poolGroup, spaGroup) {
  ...
  const updatePoolGeometryClip = (spa) => {
    ...
    if (spa) {
      ...
      // Blue = pool wall below coping, plus anything intruding inside the spa interior.
      const blueMinZ = copingUnderZ - 0.30 - pad;
      const blueMaxZ = copingUnderZ + 0.01;
      const blueThroatVolume = { minX, maxX, minY, maxY, minZ: blueMinZ, maxZ: blueMaxZ };
      const mergedBlueVolume = mergeWorldBoxes(blueThroatVolume, getSpaInteriorClearBox(spa));
      bluePlanes = [
        new THREE.Plane(new THREE.Vector3(-1, 0, 0), mergedBlueVolume.minX),
        new THREE.Plane(new THREE.Vector3( 1, 0, 0), -mergedBlueVolume.maxX),
        new THREE.Plane(new THREE.Vector3( 0,-1, 0), mergedBlueVolume.minY),
        new THREE.Plane(new THREE.Vector3( 0, 1, 0), -mergedBlueVolume.maxY),
        new THREE.Plane(new THREE.Vector3( 0, 0,-1), mergedBlueVolume.minZ),
        new THREE.Plane(new THREE.Vector3( 0, 0, 1), -mergedBlueVolume.maxZ)
      ];
