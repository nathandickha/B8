...
const SPA_THROAT_DEBUG_YELLOW_ALONG_PAD = -0.50;
const SPA_THROAT_DEBUG_YELLOW_EXTRA_ALONG_WALL = -0.50;

const SPA_THROAT_DEBUG_BLUE_COLOR = 0x00b7ff;
const SPA_THROAT_DEBUG_BLUE_DEPTH = 0.2;
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  // 3) Pool throat / wall clip region (yellow + blue visually locked to actual throat clip XY)
  ...
  if (throat) {
    ...
    // IMPORTANT: use the actual throat clip box for XY so the yellow and blue
    // debug volumes visually match the real wall/coping clip region.
    ...
    // Yellow: same XY as actual throat clip, top referenced to spa top / pool top logic.
    ...
    // Blue: same XY as actual throat clip, top locked to coping underside / pool wall height.
...
export function updatePoolWaterVoid(poolGroup, spaGroup) {
  ...
  const updatePoolGeometryClip = (spa) => {
    ...
    if (spa) {
      ...
      const yellowMinZ = copingUnderZ - pad;
      const yellowMaxZ = Math.max(poolTopZ + pad, sz + sh * 0.5 + pad);
      yellowPlanes = [
      ...
      copingMeshSet = new Set(getAllPoolCopingMeshes(poolGroup));
    }
