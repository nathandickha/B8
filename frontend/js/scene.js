...
const SPA_THROAT_ALONG_PAD = 0.12; // extend throat clip past spa extents along the wall
const SPA_THROAT_WIDTH_EXTRA = 0.10; // widen throat clip across the wall opening

// Stable throat controls
const SPA_THROAT_YELLOW_LENGTH_SHRINK = 0.030; // shorten coping cut along wall per side
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  // 3) Pool throat / wall clip region (yellow = coping, blue = wall)
  ...
  if (throat) {
    ...
    const yellowThroat = shrinkThroatBoxAlongWall(throat, spaGroup, SPA_THROAT_YELLOW_LENGTH_SHRINK) || throat;
    const blueThroat = shrinkThroatBoxAlongWall(throat, spaGroup, SPA_THROAT_BLUE_LENGTH_SHRINK) || throat;
    ...
    // Blue: wall-only volume below coping.
...
export function updatePoolWaterVoid(poolGroup, spaGroup) {
  ...
  const updatePoolGeometryClip = (spa) => {
    ...
    if (spa) {
      ...
      const yellowClipBox = shrinkThroatBoxAlongWall(clipBox, spa, SPA_THROAT_YELLOW_LENGTH_SHRINK) || clipBox;
      const blueClipBox = shrinkThroatBoxAlongWall(clipBox, spa, SPA_THROAT_BLUE_LENGTH_SHRINK) || clipBox;
      ...
      const yellowMaxZ = poolTopZ + 0.02;
      yellowPlanes = [
        new THREE.Plane(new THREE.Vector3(-1, 0, 0), yellowClipBox.minX),
        new THREE.Plane(new THREE.Vector3( 1, 0, 0), -yellowClipBox.maxX),
        new THREE.Plane(new THREE.Vector3( 0,-1, 0), yellowClipBox.minY),
        new THREE.Plane(new THREE.Vector3( 0, 1, 0), -yellowClipBox.maxY),
      ...
      const blueMaxZ = copingUnderZ + SPA_THROAT_DEBUG_BLUE_TOP_PAD;
      bluePlanes = [
        new THREE.Plane(new THREE.Vector3(-1, 0, 0), blueClipBox.minX),
        new THREE.Plane(new THREE.Vector3( 1, 0, 0), -blueClipBox.maxX),
        new THREE.Plane(new THREE.Vector3( 0,-1, 0), blueClipBox.minY),
        new THREE.Plane(new THREE.Vector3( 0, 1, 0), -blueClipBox.maxY),
